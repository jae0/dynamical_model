...
# Size-structured dynamical model for snow crab fishery 
## Maritimes Region of Canada

------------------------------
## NOTE:

- this is JULIA (https://julialang.org/) code and not R 
...
If data files have not already been created, then do so. Data files are generated by either:
  ...
  - bio.snowcrab::03.snowcrab_index_carstm.r

or 

  - bio.snowcrab::03.snowcrab_index_sizestructured_carstm.r  
...
------------------------------
## RUN LEVEL OPTIONS: define time, area model and model-type:
...
# Define sampler and initiate it
n_sample_test = 30
n_adapts_test = 30
n_chains_test = 4
...
# sample from model using sampler
...
Now that we have Turing run parameters that work reasonably and a model form that seem viable, run for production of parameter estimates.
