...
# y = observed cases (integer)
# log_offset = log( offset )
# W = adjacency matrix ## n X n (binary)
# X = modelmatrix  ## scaled_x <- c(scale(x)); model.matrix(~scaled_x )

p = size(X)[2]  # number of coefficients
n = size(X)[1]
...
W_rowsums = vec( sum(W, dims=2) );
D = diagm(W_rowsums)
  
@model function modelCAR(D, W, X, log_offset, y)
    # base model .. slow
...
@model function modelCAR_prec(D, W, X, log_offset, y)
    alpha ~ Uniform(0.0, 1.0); # alpha = 0.9 ; alpha==1 for BYM / iCAR
    tau ~ Gamma(2.0, 1.0/2.0);  # tau=0.9
    beta ~ filldist( Normal(0.0, 1.0), p);
...
@model function modelCAR_sparse(D, W, X, log_offset, y)
    alpha ~ Uniform(0.0, 1.0); # alpha = 0.9 ; alpha==1 for BYM / iCAR
    tau ~ Gamma(2.0, 1.0/2.0);  # tau=0.9
    beta ~ filldist( Normal(0.0, 1.0), p);
...
m = modelCAR(D, W, X, log_offset, y)        # 161 sec

m = modelCAR_prec(D, W, X, log_offset, y)   # 50 sec

m = modelCAR_sparse(D, W, X, log_offset, y)
  
# check timings and accuracy
n_samples, n_adapts, n_chains = 100, 100, 1
target_acceptance, max_depth, init_ϵ = 0.65, 9, 0.05
...
# larger runs (linux)
n_samples, n_adapts, n_chains = 1_000, 5_000, 4
target_acceptance, max_depth, init_ϵ = 0.65, 9, 0.05
turing_sampler = Turing.NUTS(n_adapts, target_acceptance; max_depth=max_depth, init_ϵ=init_ϵ)
